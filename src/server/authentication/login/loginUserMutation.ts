...
const errorLoginFailed: Omit<TRPCError, 'name'> = {
  code: 'NOT_FOUND',
...
export const loginUserSchema = z.object({
  ...
  password: z.string().trim().max(72, 'Invalid password'),
});
...
export async function loginUserMutation({
  input,
  ctx,
}: MutationPropsWithInput<LoginInputType>) {
  const { res } = ctx;
  const { email, password } = input;

  try {
    const result = await getPool().query(SQL_GET_USER_BY_EMAIL, [email]);
    const user = parseDBQueryResult(result, LoginUserSchema);

    if (!user) {
      throw new TRPCError(errorLoginFailed);
    }
