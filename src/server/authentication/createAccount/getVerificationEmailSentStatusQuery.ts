...
type GetVerificationEmailSentStatusInput = z.infer<
  typeof GetVerificationEmailSentStatusSchema
>;
...
export async function getVerificationEmailSentStatusQuery(
  ...
  for (let attempts = 0; attempts < 4; attempts++) {
    ...
    const result = await getPool().query(
      SQL_GET_VERIFICATION_EMAIL_SEND_STATE,
      [userId]
    );

    const { email_sent_status } =
      parseDBQueryResult(result, EmailSentStatusSchema) || {};

    if (
      !email_sent_status ||
      email_sent_status !== EmailSentStatus.emailQueued
    ) {
