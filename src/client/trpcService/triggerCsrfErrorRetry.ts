...
const csrfRetryErrorMessage = 'Unable to establish secure session. Please try again later.';

interface TriggerCsrfErrorRetryProps { 
  onRetryFailure: (props: { message: string }) => void; 
}

export function triggerCsrfErrorRetry ({ onRetryFailure }: TriggerCsrfErrorRetryProps) {
