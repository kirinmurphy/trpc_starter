...
import {
  PasswordVerificationNotificationType,
  passwordVerificationNotifications,
} from './requestPasswordNotifications';
...
export function RequestResetPasswordEmail() {
  const notificationType =
    useNotificationQueryParam<PasswordVerificationNotificationType>({
  ...
  const { mutate, error, isLoading } =
    trpcService.auth.requestResetPasswordEmail.useMutation({
      onSuccess: () => {
        setIsEmailSent(true);
      },
    });
      ...
      {!isEmailSent && (
        ...
        >
          {({ fieldErrors }) => (
            <>
              <InlineNotification
                {...passwordVerificationNotifications[notificationType]}
              />
      ...
      {isEmailSent && (
        <div>
          ...
          <p className="max-w-[370px]">
            If there is an account associated with this email, we will send you
