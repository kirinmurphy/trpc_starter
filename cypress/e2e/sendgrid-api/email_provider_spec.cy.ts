describe('SendGrid API Email Integration Tests', () => {
  ...
  // if (testMode === 'sendgrid-api') {
  //   const testUser = {
  //     name: 'sendgridtestuser',
  //     email: `sendgrid.testuser@example.com`, // Using a fixed email now
  //     password: 'SendGridPassword123!',
  //   };

  //   before(() => {
  //     cy.log(`Running SendGrid API tests in mode: ${testMode}`);
  ...
  //   it('should trigger a SendGrid API email on successful user registration and verify its payload', () => {
  ...
  //       );
  //       // You can re-assert on interception.request.body if needed, but it's already done in intercept callback
  //     });
  //   });
  // }
  // } else {
  //   it.skip(`(SKIPPED) SendGrid API tests are not applicable for the current mode: ${testMode}`, () => {});
  // }
});
